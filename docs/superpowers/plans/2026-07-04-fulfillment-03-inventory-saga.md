...
## AMENDMENT C (user directive, 2026-07-04): signature-driven dispatch — the aggregate IS the routing table

Replaces Amendment B's route-map array. Task 9.

- **`SharedKernel/AggregateRoot` interface**: `apply(object $event): void`; `/** @return list<object> */ public function releaseEvents(): array;`. All three aggregates implement it (SharedKernel is Domain-legal).
- **Aggregates take the command object**: intention methods refactor from unpacked args to the message itself — `Order::place(PlaceOrder $command)`, `cancel(CancelOrder $command)`, `markStockReserved(MarkStockReserved $command)`; `InventoryItem::restock(Restock)`, `reserve(ReserveStock)`, `release(ReleaseReservation)`; `FulfillmentProcess::start(OrderPlaced)`, `confirmReservation(StockReserved)`, `rejectReservation(StockReservationRejected)`. Logic unchanged; domain unit tests update mechanically (construct the message).
- **`Platform/Actor/AggregateBehavior`** (replaces `AggregateEntityBehavior`, which is deleted): discovers handlers by SIGNATURE CONVENTION — every public non-static method with exactly one parameter whose declared type is a concrete class is a command handler for that class. `apply()` is auto-excluded (param type `object`), constructor/statics excluded. Reflection runs once per aggregate class (static cache keyed by class-string). Ambiguity (two methods claiming one message class) → `LogicException` at build time. Unknown message at runtime → `Effect::unhandled()`.
  - `AggregateBehavior::for(aggregate, persistenceId, accepted: Closure, rejected: Closure, eventStore, snapshotStore, publisher, passivateAfter)` — entity variant; drain/reply semantics identical to Amendment B (empty→reply accepted(current); RejectionEvent→persist+reply rejected(reason); success→persist+reply accepted($next)); wires withEventPublisher/withSignalHandler/snapshot conventions.
  - `AggregateBehavior::forProcess(aggregate, persistenceId, sideEffects: Closure(TAgg $next, list<object> $persisted): void, eventStore, snapshotStore, passivateAfter)` — saga variant: no replies, no publisher; sideEffects runs in thenRun after persist. `FulfillmentProcessActor` adopts it — its side-effect dispatch moves into the closure, its if/elseif routing is deleted.
- **Psalm**: `@template TAgg of AggregateRoot` binding must eliminate the Amendment-B `Mixed*` suppressions (reviewer-verified feasible); at most one narrowly-commented suppress for the dynamic invoke if `ReflectionMethod::invoke` doesn't satisfy level 1.
- **Frozen**: all integration tests pass UNMODIFIED; unit tests change only mechanically (message construction). No packages/ changes. Deptrac layers unchanged (same PlatformActor directory).
- Endgame note (for the tutorial + future framework work): `AggregateRoot` + `AggregateBehavior` are graduation candidates for a first-class Nexus DDD package.
