...
### Task 19: nexus-messenger-console — Symfony Console runners (user-requested addition 2026-07-03, separate package confirmed by user)

**Why a separate package:** the spec (§2) deliberately keeps `nexus-messenger` free of `symfony/console`; the repo convention for optional integrations is a dedicated package. New package `packages/nexus-messenger-console`, composer `nexus-actors/messenger-console`, namespace `Monadial\Nexus\Messenger\Console`.

**Deliverables:**
- `composer.json` — requires php >=8.5.7, `nexus-actors/core`, `nexus-actors/runtime`, `nexus-actors/messenger`, `nexus-actors/serialization` (all dev-main), `symfony/console` `^7.4 || ^8.0`; PSR-4 `Monadial\Nexus\Messenger\Console\` → src/, tests dev autoload.
- `src/ConsumeCommand.php` — `#[AsCommand(name: 'nexus:messenger:consume', ...)]`, `final class ... extends Command`. Ctor injection: `(Runtime $runtime, ReceiverInterface $receiver, MessageRouter $router, ?Observability $observability = null, ?LoggerInterface $logger = null, ?EventDispatcherInterface $events = null)`. Options: `--receivers|-r` (int, default 1 → `MessengerBridge::spawnReceivers`), `--limit` (message count), `--memory-limit` (accepts `128M`/`1G` strings via `MemoryLimit::parse`), `--time-limit` (seconds), `--poll-interval` (ms, default 100), `--dead-letters` (flag → `UnroutablePolicy::DeadLetters` with `$system->deadLetters()`). Boots `ActorSystem::create`, wires receivers + `LifecycleWatchdog` from the limit options (no watchdog when no limits given), `$system->run()`, returns `Command::SUCCESS`. Implements `SignalableCommandInterface` (SIGINT/SIGTERM → `$system->shutdown(Duration::seconds(10))`). Output via `SymfonyStyle`: startup summary (receivers, limits, transport class) + shutdown line.
- `src/ProduceCommand.php` — `#[AsCommand(name: 'nexus:messenger:produce')]`. Ctor `(SenderInterface $sender, MessageSerializer $serializer, TypeRegistry $types)`. Args: `type` (registered type name), `body` (JSON string, materialized via `$serializer->deserialize($body, $class)` — works with Valinor serializers); option `--count` (default 1). Resolves class via `TypeRegistry::classForName()` — registry-strict, matching the bridge's decode posture; clear error on unknown type. Sends via `MessengerBridge::gateway()`.
- `src/MemoryLimit.php` — tiny readonly parser (`'256M'` → bytes; plain ints and K/M/G suffixes; throws `InvalidArgumentException` otherwise). TDD.
- Tests: unit `MemoryLimitTest` (TDD); `CommandTester`-based tests — `ProduceCommand` with a recording sender + Valinor serializer (assert N envelopes, correct message content, unknown-type error path); `ConsumeCommand` end-to-end with pre-seeded `InMemoryTransport` + `--limit=N --poll-interval=20` (watchdog ends the run; assert `Command::SUCCESS` and consumption). Keep runtimes short (these boot a FiberRuntime inside the unit suite that GrumPHP runs on every commit).
- Monorepo wiring: root composer autoload/autoload-dev; root `require` gains explicit `symfony/console: ^8.0` if not already a direct requirement; phpunit `unit` suite dir + coverage `<source>` include; deptrac layer `MessengerConsole` → Core, Runtime, Serialization, Messenger, Observability; split.yml matrix `- { local: 'nexus-messenger-console', remote: 'messenger-console' }` + `gh repo create nexus-actors/messenger-console --public`; CHANGELOG bullet; CLAUDE.md graph line + short section; package README (subtree-split template).
