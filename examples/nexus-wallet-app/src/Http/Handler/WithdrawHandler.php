...
final readonly class WithdrawHandler
{
    public function __invoke(
        ...
        $reply = $walletRef->ref
            ->ask(new Withdraw(new Money($body->amountCents)), Duration::seconds(2))
            ->await();

        assert($reply instanceof WithdrawResult);

        $dto = new WalletOperationResponse(
            ownerId: $principal->id(),
            accepted: $reply->accepted,
            balance: $reply->balanceCents,
            reason: $reply->accepted ? null : $reply->rejectionReason,
        );

        $response = JsonResponse::ok($dto);
