...
**Timers** use Swoole's native timer API. `scheduleOnce()` calls `Swoole\Timer::after($ms, $callback)`. `scheduleRepeatedly()` uses `Timer::after()` for the initial delay, then `Timer::tick()` for the recurring interval; the returned `SwooleCancellable` is retargeted at the handover, so `cancel()` always clears the live timer — before or after the first fire. Timers scheduled before `run()` return a `DeferredCancellable`: cancelled before `run()`, the timer is never created; cancelled after, it delegates to the materialised timer handle.
