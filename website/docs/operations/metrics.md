...
For a ready-made metrics/traces/logs pipeline, add **`nexus-observability-otel`**. Once observability is wired into the `ActorSystem`, the runtime emits `nexus.actor.messages.processed` and `nexus.actor.message.processing.duration`; with `nexus-cluster-tcp` you also get the `nexus.cluster.*` family (`messages.sent`, `bytes.sent`, `frames.sent`, `frames.decode_failed`, `control_send.failed`, `asks.pending`/`.resolved`/`.timed_out`/`.capacity_rejected`, `ask.duration`, `handshake.rejected`) plus `nexus.actor_system.*` gauges — all exported over OTLP to a collector (e.g. Grafana LGTM). See the [observability guide](observability.md) and `tests/Performance/distributed/README-observability.md`.
:::
