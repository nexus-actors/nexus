...
Nexus exists to make concurrent, long-running, fault-tolerant PHP systems practical without changing languages. It is opinionated: actors, supervision, immutable messages, and explicit runtime choices over ad-hoc workers and shared mutable state.
...
- Stateful integrations — payments, fulfillment, IoT, game or session backends
- Domain models where each aggregate or entity maps naturally to an actor
...
- Small applications where queue workers and cron already solve the problem
...
- **Runtime abstraction over single-runtime lock-in** — portability between `FiberRuntime`, `SwooleRuntime`, and `StepRuntime`
...
- A universal replacement for Laravel or Symfony application architecture
...
- Do we need an explicit recovery strategy per failure mode?
- Do we benefit from isolating state per entity or workflow?
...
- [Quick start](../getting-started/quick-start.md) — build a counter actor in 10 minutes
- [Actors](./actors.md) — the core abstraction in detail
- [Supervision](./supervision.md) — how Nexus handles failures
