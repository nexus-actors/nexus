...
- **`Effect`** type: Command handlers return effects instead of behaviors. `Effect::persist(events)` to persist, `Effect::none()` to skip, with `thenReply()` and `thenRun()` chaining (side-effect hooks run on every effect: with the post-persist state on the persist path, with the unchanged current state otherwise).
