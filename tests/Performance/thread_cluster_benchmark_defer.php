...
/**
 * Event::defer() benchmark: uses Swoole\Event::defer() + Coroutine::yield()/resume()
 * for ~0.86μs context switching instead of Coroutine::sleep(0.001) at ~1440μs.
 *
 * Usage:
 *   docker compose exec php-swoole-thread php tests/Performance/thread_cluster_benchmark_defer.php [workers] [seconds] [batch] [sendCoros]
 *
...
$workerScript = __DIR__ . '/thread_cluster_worker_defer.php';

echo "\n";
echo "=== Event::defer() Thread Cluster Benchmark ===\n";
echo "\n";
echo "  Workers:       {$workerCount}\n";
echo "  Duration:      {$durationSec}s\n";
echo "  Batch size:    {$batchSize}\n";
echo "  Send coros:    {$sendCoros}/worker\n";
echo "  Yield method:  Event::defer() + Coroutine::yield()/resume() (~0.86μs)\n";
echo "\n";
