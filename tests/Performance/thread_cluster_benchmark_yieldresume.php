...
/**
 * Yield/Resume benchmark: overcomes the 1ms sleep floor using explicit
 * Coroutine::yield()/resume() for ~0.2μs context switching.
 *
 * Usage:
 *   docker compose exec php-swoole-thread php tests/Performance/thread_cluster_benchmark_yieldresume.php [workers] [seconds] [batch] [sendCoros]
 *
...
$sendCoros = (int) ($argv[4] ?? 4);
$workerScript = __DIR__ . '/thread_cluster_worker_yieldresume.php';

echo "\n";
echo "=== Yield/Resume Thread Cluster Benchmark ===\n";
echo "\n";
echo "  Workers:       {$workerCount}\n";
echo "  Duration:      {$durationSec}s\n";
echo "  Batch size:    {$batchSize}\n";
echo "  Send coros:    {$sendCoros}/worker\n";
echo "  Yield method:  Coroutine::yield()/resume() (~0.2μs)\n";
echo "\n";
